    ...
    for i in range(len(insn)):
        ...
        try:
            if capstone.x86.X86_GRP_INT in insn[i].groups and "syscall" in insn[i].mnemonic:
                syscalls[insn[i].address] = i
        except:
            continue
    return syscalls


def find_syscalls(insn): #get all calls without thinking reachable
    syscalls = set()
...
def init(fname):
    with open(fname, 'rb') as f:
        elf = ELFFile(f)
        code = elf.get_section_by_name('.text')  # ---codes
        ops = code.data()                        # ---data
        addr = code['sh_addr']                   # ---start offset address
...
def main(fnames):
    ...
    for fname in files:
        ...
        for wlib in whitelist:
            if wlib in fname:   #check call in list of dso path?? what is he doing
