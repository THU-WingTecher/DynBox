...
Applications = ['httpd-bit', 'httpd-dru', 'nginx', 'redis','memcached', 'tar-czvf', 'tar-test', 'tar-xvzf']
...
def initDicts():
    for key, value in countC2C.items():
        countC2C[key] = {
            "tot" : 0,
            "succ" : 0,
            "fail" : 0
...
def processC2C(requireCalls, targetApp, log_file):
    ...
    # print(countC2C)
    outputRes("Results of C2C on " + targetApp, countC2C, repeat_count, log_file)

    res_str = f"Permitted Syscall Number of C2C on {targetApp}: {len(requireCalls)}"
    print(res_str, "\n")
    log_file.write(res_str+"\n")
    log_file.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--syscall', "-s", required=True, help="Permitted syscalls of C2C")
    ...
    resultDir = os.path.join(args.result, "C2C")
    ...
    if args.target == "all":
        ...
        for app in Applications:
            initDicts()
            log_file = open(os.path.join(resultDir, app+".C2C"), "w")
            callFile = os.path.join(__location__, "permittedSyscall", app)
            with open(callFile) as f:
                calls = json.load(f)
            processC2C(calls, app, log_file)
            
            
    else:
        log_file = open(os.path.join(resultDir, args.target+".C2C"), "w")
        callFile = os.path.join(__location__, "permittedSyscall", args.target)
        with open(callFile) as f:
            calls = json.load(f)
        # print(calls)
        initDicts()
        processC2C(calls, args.target, log_file)